...
{\bf 4.3. Feature shrink an propagation.} The object and face features are computed frame by frame. We use the 
...
{\bf 4.4. Normalization.} Working with multi-modal features requires of a normalization. We apply the linear mapping of each feature to the range $[0,1)$ using the minimum and maximum values of each feature vector.\\
...
\begin{figure}[h]
     \begin{center}
          ...
          \subfigure[Weight distribution of the features learned by the model. Each graph represents and an independent experiment, still the inter experiments weight relationship is similar.]{%
           \includegraphics[width=1\linewidth]{Image/weights.pdf} 
           \label {fig:first}
           }
        \subfigure[Average weights inter models. The results show that the most discriminative feature ha a strong relationship with the handled object, followed by the detection of faces.]{%
           \includegraphics[width=1\linewidth]{Image/individual.pdf} 
           \label {fig:second}
           }
          \end{center}
    \caption{todo... 
